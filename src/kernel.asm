...
get_input:
	...
	mov ax, 0x200				; reset ES & DS segments to kernel area
	mov es, ax
	mov ds, ax
...
found_pgm:
	...
	mov ax, 0x800				; memory location to load pgm to
	mov es, ax
	mov al, bl 					; number of sector to read
	xor bx, bx					; ES:BX -> 0x800:0x0000
...
run_program:
	...
	mov ax, 0x800				; pgm loaded, set segment reg to location
	mov ds, ax
	mov es, ax
	mov fs, ax
	mov gs, ax
	jmp 800h:0000h					; far jmp to pgm
